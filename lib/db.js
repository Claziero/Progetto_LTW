...
async function loadMainListing() {
    ...
    var query = "SELECT Titolo, Tipo, DataOra, PostiDisponibili, Descrizione, ID, Disponibile,\
                ...
                FROM Event WHERE Disponibile = true AND Dataora <= \'" + mese + "\'\
                ORDER BY ID;";
...
async function loadHighlights() {
    ...
    var query = "SELECT Titolo, Tipo, DataOra, PostiDisponibili, Descrizione, ID, Disponibile,\
                Immagine, Luogo, Organizzatore\
                FROM Event WHERE Disponibile = true AND DataOra >= \'" + oggi + "\'\
...
async function loadNext() {
    ...
    var query = "SELECT Titolo, Tipo, DataOra, PostiDisponibili, Descrizione, ID, Disponibile,\
                Immagine, Luogo, Organizzatore\
                FROM Event WHERE Disponibile = true AND DataOra >= \'" + mese + "\' ORDER BY ID;";
...
async function loadPassed() {
    ...
    var query = "SELECT Titolo, Tipo, DataOra, PostiDisponibili, Descrizione, ID, Disponibile,\
                Immagine, Luogo, Organizzatore\
                FROM Event WHERE Passato = true ORDER BY Dataora;";
...
async function changeUser(user) {
    ...
    var pwdQuery = "SELECT Passwd FROM Users WHERE Email = \'" + user.email + "\';";
...
async function logUser(user) {
    ...
    var query = 'SELECT passwd, nome FROM Users WHERE email = \'' + user.email + '\';';
...
async function getLevel(email) {
    // Imposta la query
    var query = 'SELECT privilegi FROM Users WHERE email = \'' + email + '\';';
...
async function book(id, user) {
    ...
    var query = "SELECT * FROM Booking WHERE Email_User = \'" + user.email +
        "\' AND ID_Event = " + id;
    ...
    try {
        ...
        if (res.length == 0) {
            ...
            query = "SELECT PostiDisponibili FROM Event WHERE ID = " + id;
...
async function search(src) {
    ...
    var query = "SELECT Titolo, Tipo, DataOra, PostiDisponibili, Descrizione, ID, Immagine,\
                ...
                FROM Event WHERE Titolo ILIKE \'%" + src + "%\' ORDER BY ID;"
...
async function prenotazioni(user) {
    // Imposta la query
    var query = "SELECT Titolo, Event.DataOra, ID FROM Event JOIN Booking\
                ON ID_Event = ID WHERE Disponibile = true AND Email_User =\'" + user + "\';"
...
async function storico(user) {
    // Imposta la query
    var query = "SELECT Titolo, Event.DataOra, Luogo FROM Event JOIN Booking\
                ON ID_Event = ID WHERE Passato = true AND Email_User =\'" + user + "\';"
...
async function organizzazioni(org) {
    // Imposta la query
    var query = "SELECT Titolo, DataOra, ID FROM Event\
                WHERE Organizzatore =\'" + org + "\' AND Disponibile = True;"
...
async function beOrganizer(email) {
    // Imposta la query
    var query = "UPDATE Users SET Privilegi = 3 WHERE Email = \'" + email + "\';";
...
async function getRequests() {
    // Imposta la query
    var query = 'SELECT Nome, Cognome, Email FROM Users WHERE Privilegi = 3;';
...
async function acceptRequest(email) {
    // Imposta la query
    var query = "UPDATE Users SET Privilegi = 1 WHERE Email = \'" + email + "\';";
...
async function declineRequest(email) {
    // Imposta la query
    var query = "UPDATE Users SET Privilegi = 0 WHERE Email = \'" + email + "\';";
...
async function unbook(id, user) {
    // Imposta la query
    var query = "DELETE FROM Booking WHERE Email_User =\'" + user + "\'\
                AND ID_Event =" + id + ";"
...
async function deleteUser(email) {
    ...
    var remBooks = 'DELETE FROM Booking WHERE Email_User = \'' + email + '\';';
...
async function getUserData(email) {
    // Imposta la query
    var query = "SELECT Nome, Cognome, Datanascita FROM Users WHERE email = \'" + email + "\';"
...
async function getPrintData(id, user) {
    ...
    var query = "SELECT Nome, Cognome, Email, ID, Titolo, Luogo,\
                Event.DataOra AS edataora, Booking.DataOra AS pdataora\
                FROM Event JOIN Booking ON ID_Event = ID\
                JOIN Users ON Email_User = Email\
