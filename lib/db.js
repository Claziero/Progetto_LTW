...
async function loadMainListing() {
    ...
    var query = "SELECT Title, Type, DateTime, Seats, Description, ID, Available,\
                Image, Location, Organizer\
                FROM Event WHERE Available = true AND DateTime <= \'" + mese + "\'\
                ORDER BY ID;";
...
async function loadHighlights() {
    ...
    var query = "SELECT Title, Type, DateTime, Seats, Description, ID, Available,\
                Image, Location, Organizer\
                FROM Event WHERE Available = true AND DateTime >= \'" + oggi + "\'\
                AND DateTime <= \'" + sett + "\' ORDER BY ID;";
...
async function loadNext() {
    ...
    var query = "SELECT Title, Type, DateTime, Seats, Description, ID, Available,\
                Image, Location, Organizer\
                FROM Event WHERE Available = true AND DateTime >= \'" + mese + "\' ORDER BY ID;";
...
async function loadPassed() {
    ...
    var query = "SELECT Title, Type, DateTime, Seats, Description, ID, Available,\
                Image, Location, Organizer\
                FROM Event WHERE Passed = true ORDER BY DateTime;";
...
async function getMaxID() {
    var query = 'SELECT max(id) FROM Event;';
...
async function getImagesList() {
    var query = 'SELECT Name, Title FROM ImgMapping;';
...
async function deleteEvent(eventID) {
    ...
    var remUser = 'DELETE FROM Booking WHERE ID_Event = ' + eventID + ';';

    // Imposta la query
    var query = 'DELETE FROM Event WHERE id = ' + eventID + ';';
...
async function changeUser(user) {
    ...
    var pwdQuery = "SELECT Password FROM Users WHERE Username = \'" + user.username + "\';";
    ...
    try {
        ...
        if (cjs.MD5(user.oldpwd).toString() == pwd[0].password) {
            // Query per l'inserimento
            var query = 'UPDATE Users SET ';
            query += 'Name = \'' + user.name + '\',';
            query += 'Surname = \'' + user.surname + '\',';
            query += 'Born = \'' + user.born + '\',';
            query += 'Password = \'' + cjs.MD5(user.newpwd).toString() + '\' ';
...
*   name utente = Credenziali corrette
*   -1 = Password errata
*   -2 = Users non registrato
*/
async function logUser(user) {
    // Query per la ricerca della password
    var query = 'SELECT password, name FROM Users WHERE username = \'' + user.username + '\';';
    ...
    try {
        ...
        if (res.length > 0) {
            ...
            if (res[0]['password'] == hash) return res[0]['name'];
            else return -1;
        }
        else return -2;
    }
...
// Funzione per prendere il livello di privileges di un utente
async function getLevel(username) {
    // Imposta la query
    var query = 'SELECT privileges FROM Users WHERE username = \'' + username + '\';';
 ...
 *  -3 = Users già registrato all'evento
*/
async function book(id, user) {
    // Controlla che la persona non sia già prenotata per questo evento
    var query = "SELECT * FROM Booking WHERE Username_User = \'" + user.username +
        "\' AND ID_Event = " + id;
    ...
    try {
        ...
        if (res.length == 0) {
            ...
            query = "SELECT Seats FROM Event WHERE ID = " + id;
...
async function search(src) {
    // Imposta la query
    var query = "SELECT Title, Type, DateTime, Seats, Description, ID, Image,\
                Location, Organizer, Available\
                FROM Event WHERE Title ILIKE \'%" + src + "%\' ORDER BY ID;"
...
async function prenotazioni(user) {
    // Imposta la query
    var query = "SELECT Title, Event.DateTime, ID FROM Event JOIN Booking\
                ON ID_Event = ID WHERE Available = true AND Username_User =\'" + user + "\';"
...
async function storico(user) {
    // Imposta la query
    var query = "SELECT Title, Event.DateTime, Location FROM Event JOIN Booking\
                ON ID_Event = ID WHERE Passed = true AND Username_User =\'" + user + "\';"
...
// Funzione per la visualizzazione degli eventi creati (organizer)
async function organizzazioni(org) {
    // Imposta la query
    var query = "SELECT Title, DateTime, ID FROM Event\
                WHERE Organizer =\'" + org + "\' AND Available = True;"
...
// Funzione per la richiesta di diventare organizer
async function beOrganizer(username) {
    // Imposta la query
    var query = "UPDATE Users SET Privileges = 3 WHERE Username = \'" + username + "\';";
...
async function getRequests() {
    // Imposta la query
    var query = 'SELECT Name, Surname, Username FROM Users WHERE Privileges = 3;';
...
async function acceptRequest(username) {
    // Imposta la query
    var query = "UPDATE Users SET Privileges = 1 WHERE Username = \'" + username + "\';";
...
async function declineRequest(username) {
    // Imposta la query
    var query = "UPDATE Users SET Privileges = 0 WHERE Username = \'" + username + "\';";
...
async function unbook(id, user) {
    // Imposta la query
    var query = "DELETE FROM Booking WHERE Username_User =\'" + user + "\'\
                AND ID_Event =" + id + ";"
...
async function deleteUser(username) {
    ...
    var remBooks = 'DELETE FROM Booking WHERE Username_User = \'' + username + '\';';
...
async function getUserData(username) {
    // Imposta la query
    var query = "SELECT Name, Surname, Datanascita FROM Users WHERE username = \'" + username + "\';"
...
async function getPrintData(id, user) {
    // Imposta la query
    var query = "SELECT Name, Surname, Username, ID, Title, Location,\
                Event.DateTime AS edataora, Booking.DateTime AS pdataora\
                FROM Event JOIN Booking ON ID_Event = ID\
                JOIN Users ON Username_User = Username\
